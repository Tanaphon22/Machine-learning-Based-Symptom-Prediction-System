...
4. **Model Evaluation**
   ...
   ในการประเมินผลโมเดลนี้ เราเลือกใช้ Precision, Recall และ F1-score เป็นตัววัด โดยผลจาก test set แสดงว่า Macro avg Precision = 0.46, Macro avg Recall = 0.55, Macro avg F1-score = 0.47 และ train set แสดงว่า Macro avg Precision = 0.56, Macro avg Recall = 0.73, Macro avg F1-score = 0.59 ซึ่งสะท้อนว่าโมเดลยังไม่สามารถนำไปใช้งานจริงได้ ปัญหาอาจเกิดจากจำนวนคลาสที่มากเกินไปเมื่อเทียบกับขนาดข้อมูล และความไม่สมดุลของจำนวนข้อมูลในแต่ละคลาส นอกจากนี้ การมีคำที่มีความหมายใกล้เคียงกันแต่รูปคำต่างกัน เช่น 'ไข้' กับ 'ตัวร้อน' ก็ทำให้โมเดลสับสน แม้จะใช้วิธี Duplicate data หรือปรับ sample weight แล้ว แต่ผลลัพธ์ก็ยังไม่ดีนัก

   
   <img width="968" height="884" alt="image" src="https://github.com/user-attachments/assets/e1032868-2d62-489a-b5be-fe6896fdbac4" />
...
6. **Model Tuning & Optimization**
   ...
   ในโมเดลนี้ได้ทำการปรับค่า threshold ระหว่าง 0.1-0.9 ของแต่ละคลาสให้เหมาะสมและได้ค่า F1-score ที่สูงที่สุด

7. **Deployment & Monitoring**
   
   ในหัวข้อนี้เราจะมาลองใช้งานโมเดลเพื่อให้ทำนายอาการป่วยออกมา

   <img width="514" height="259" alt="image" src="https://github.com/user-attachments/assets/bf1020fd-81c8-4aff-8399-deaf578687a4" />

   จากภาพจะเห็นได้ว่า เมื่อป้อนอาการเพียงแค่ 1 อาการ โมเดลสามารถทำนายอาการนั้นได้ถูกต้อง พร้อมทั้งยังแสดงอาการที่มีความน่าจะเป็นรองลงมาอีก 5 อาการ 
